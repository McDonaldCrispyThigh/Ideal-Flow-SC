...
The downtown commercial core (obtained from OSMnx~\citep{BG17} landuse data,
6-vertex convex hull, $0.82$\,km$^2$) is mapped to $\HH$ via $f^{-1}$; its
pre-image is enclosed by a circle of radius $a$ centred at $\zeta_0 \in \HH$.
The Milne-Thomson dipole in~\eqref{eq:urban} enforces no-penetration on this
circle, while the image term $U a^2 / (\bar{\zeta} - \bar{\zeta}_0)$ preserves
$\psi = 0$ on $\RR$. Figure~\ref{fig:urban} (Appendix~\ref{app:figures}) shows
the resulting complex potential mapped to $\Omega$. The achieved separation
ratio $\Imag(\zeta_0) / a \approx 5.36$ gives an approximation error of about
$3.5\%$. The deflection of streamlines around the urban core is most clearly
visible in Figure~\ref{fig:comparison}, panel~(c).
...
\begin{figure}[H]
  ...
  vertices (2057\,m); blue triangles: low-elevation sink vertices (1576\,m).
  Streamlines deflect eastward relative to the uniform baseline, reproducing
  the slope-driven wind pattern characteristic of Boulder's foothill terrain.}
  \label{fig:terrain}
\end{figure}
...
\begin{figure}[H]
  ...
  on the enclosing circle; the image term at $\bar{\zeta}_0$ preserves
  $\psi = 0$ on $\partial \HH$. The achieved separation ratio
  $\Imag(\zeta_0) / a \approx 5.36$ yields an approximation error of $\sim 3.5\%$.
  See Figure~\ref{fig:comparison}(c) for the deflection pattern around the
  urban core.}
  \label{fig:urban}
\end{figure}
...
\begin{figure}[H]
  ...
  $W = U\zeta$, shown without streamlines for clarity. Each curve is the
  forward image of a vertical half-line $\Real(\zeta) = \xi_0$ in $\HH$.}
...
\begin{figure}[H]
  ...
  \caption{Three-way comparison: uniform, terrain-corrected, and
  urban-obstacle flow at identical contour levels. The progressive enrichment
  of the physical model is apparent in the streamline morphology of each panel.}
  \label{fig:three_way}
\end{figure}
